...
public class GameMap {
	...
	public GameMap(GameEngine engine, TileCoord size) {
		...
		for (int p = 0; p <= size.getP(); p++) {
			for (int q = 0; q <= size.getQ(); q++) {
				tiles.put(HexMath.cubeFromTile(new TileCoord(p, q)),
	...
	public GameMap(GameEngine engine, int radius) {
		this(engine);
		List<CubeCoord> tileList = HexMath.inRange(new CubeCoord(0, 0, 0),
				radius);
	 ...
	 * 
	 * @param coord
	 * @return an immutable list of the current entities at a given tile. will
	 *         not update to reflect changes on the map
	 */
	public List<Entity> entitiesAt(CubeCoord coord) {
		return Collections.unmodifiableList(new ArrayList<Entity>(tiles
				.get(coord)));
	}
	...
	public void removeEntity(CubeCoord coord, Entity e) {
		...
		e.setLocation(null);
		gameEngine.getEventDispatch()
				.dispatch(new MapRenderStateChangedEvent());
	}
